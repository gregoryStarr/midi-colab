      ...
      {isWebContainerStarted ? (
        <>
          ...
          <div className="w-full h-full flex items-center justify-center p-8">
            <svg
              viewBox="0 0 800 300"
              className="w-full h-full drop-shadow-[0_0_15px_rgba(64,169,255,0.6)]"
            >
              <defs>
                <linearGradient
                  id="neonGradient"
                    ...
                    ? Array.from(networkActiveKeys)
                        .map((_key, i) => {
                          ...
                          for (let j = 0; j <= nodes; j++) {
                            const x =
                              i * (800 / Math.max(networkActiveKeys.size, 1)) +
                              (j *
                                (800 / Math.max(networkActiveKeys.size, 1))) /
                                nodes;
                            ...
                            const y =
                              ...
                              Math.sin(phase * 0.5) *
                                amp *
                                Math.sin((j / nodes) * Math.PI); // Envelope
                            points += `L ${x} ${y} `;
                          }
                          return points;
                        })
        ...
        <div className="text-center z-10">
          ...
          <p className="text-gray-400 mb-6">
            Start the high-performance audio engine
          </p>
          <div className="flex justify-center">
            <WebContainerStarter
              onStarted={onStartEngine}
