      ...
      <div className="border-t border-white/10 pt-4 mt-auto">
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <span
              className={isEngineReady ? "text-blue-400" : "text-yellow-400"}
            >
              ●
            </span>
          </div>
        ...
        {users.length > 0 && (
          <div className="flex -space-x-2 mt-2 overflow-hidden">
            {users.map((u, i) => (
              <div
                key={i}
                className="w-6 h-6 rounded-full bg-gray-700 border border-[#0f0f13] flex items-center justify-center text-[10px] text-white"
                title={u.name}
              >
                {u.name[0]}
              </div>
